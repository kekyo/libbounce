 ...
 * - Parker threads only wait for bounce wake/shutdown, drain that queue, and
 *   execute completions on the parked thread context.
 * - fd readiness stays backend-local. Fixed waiter shards call `poll()`, claim
 *   the corresponding completion items, and enqueue them into the shared ready
 *   queue.
