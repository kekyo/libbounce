...
`libbounce::bounce`が`BOUNCE_CORE`を所有するクラスですが、通常はこのインスタンスを生成後、`set_default()`でTLSに登録します。
すると、継続関数内で、 `static libbounce::bounce::get_current()` を使用して簡単にこのインスタンスにアクセスすることが出来ます。
継続関数は必ずパーキングスレッドで呼び出されるため、いつでも `get_current()` で参照可能になります。

`libbounce::bounce`のインスタンスをグローバルに公開することは、特定のパーキングスレッドを明示的に使用したい場合に限られるため、
一般的にそのような実装を行う理由はありません。
