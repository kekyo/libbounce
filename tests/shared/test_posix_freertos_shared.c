...
static bool test_file_io_uring_available(const BOUNCE_CORE *bounce) {
  return (bounce != NULL) &&
         bounce->linux_unified_wait_enabled &&
         (bounce->io_uring_ring != NULL);
}
...
static void test_file_io_uring_write_retries_eintr_until_success(void) {
  static const char payload[] = "libbounce file io_uring EINTR write payload";
  BOUNCE_CORE bounce;
...
static void test_file_io_uring_read_retries_eintr_until_success(void) {
  static const char payload[] = "libbounce file io_uring EINTR read payload";
  BOUNCE_CORE bounce;
  BOUNCE_FILE_IO operation;
  TEST_PARK_THREAD_CONTEXT park_context;
  TEST_COMPLETION_CONTEXT completion_context;
...
static void test_file_io_uring_flush_retries_eintr_until_success(void) {
  static const char payload[] = "libbounce file io_uring EINTR flush payload";
  BOUNCE_CORE bounce;
  BOUNCE_FILE_IO operation;
  TEST_PARK_THREAD_CONTEXT park_context;
  TEST_COMPLETION_CONTEXT completion_context;
...
static void test_file_io_uring_non_eintr_error_is_reported(void) {
  static const char payload[] = "libbounce file io_uring non-EINTR payload";
  BOUNCE_CORE bounce;
  BOUNCE_FILE_IO operation;
  TEST_PARK_THREAD_CONTEXT park_context;
  TEST_COMPLETION_CONTEXT completion_context;
...
#define TEST_APPEND_CASE(function_name)                                          \
  ...
  TEST_APPEND_CASE(test_cpp_promise_immediate_child_value_runs);
...
#if defined(__linux__) && defined(LIBBOUNCE_ENABLE_FILE_IO_URING_TEST_HOOKS)
    TEST_APPEND_CASE(test_file_io_uring_write_retries_eintr_until_success);
    TEST_APPEND_CASE(test_file_io_uring_read_retries_eintr_until_success);
    TEST_APPEND_CASE(test_file_io_uring_flush_retries_eintr_until_success);
    TEST_APPEND_CASE(test_file_io_uring_non_eintr_error_is_reported);
    TEST_APPEND_CASE(test_file_io_uring_retry_preserves_cancellation);
#endif
